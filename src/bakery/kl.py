...
def topk_jsd(
    ...
    """Mixed forward/reverse KL on the teacher's top-k support.

        loss = (1 - β) · D_KL(P_teacher || P_student)
             +       β · D_KL(P_student || P_teacher)

    β = 0   → pure forward KL (teacher → student); mode-covering; default bakery
    β = 1   → pure reverse KL (student → teacher); mode-seeking; on-policy GKD
    β = 0.5 → symmetric (JSD-style) average

    This is the same convention TRL's `GKDTrainer` uses for its `beta` knob —
    monotonic in β, β=0 reduces exactly to forward KL, β=1 to reverse KL.

    Both KL terms are computed on the teacher's top-k support, with student and
    teacher each renormalized over those K so the comparison is on the same
    support. Computed in log space.
    """
    if beta < 0.0 or beta > 1.0:
        raise ValueError(f"beta must be in [0, 1], got {beta}")

    s_lp_full = F.log_softmax(student_logits / temperature, dim=-1)
    s_lp_topk = torch.gather(s_lp_full, dim=-1, index=teacher_topk_indices)
    s_lp_topk = s_lp_topk - s_lp_topk.logsumexp(dim=-1, keepdim=True)

    t_lp = teacher_topk_logprobs  # already renormalized over top-k

    # Forward KL: sum p_t · (log p_t - log p_s)
    kl_forward = (t_lp.exp() * (t_lp - s_lp_topk)).sum(dim=-1)  # (B, T)
    # Reverse KL: sum p_s · (log p_s - log p_t)
    kl_reverse = (s_lp_topk.exp() * (s_lp_topk - t_lp)).sum(dim=-1)
    mixed = (1.0 - beta) * kl_forward + beta * kl_reverse  # (B, T)

    mask_f = mask.float()
    masked = mixed * mask_f
    if per_sample:
        n = mask_f.sum(dim=-1).clamp(min=1.0)
        return masked.sum(dim=-1) / n
    n = mask_f.sum()
    return masked.sum() / n if n > 0 else masked.sum()
